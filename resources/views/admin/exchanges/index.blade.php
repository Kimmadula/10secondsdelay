...
@section('subtitle', 'Manage user skill exchanges and trades')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">All Exchanges</h3>
            <p class="text-sm text-gray-600 mt-1">Manage and monitor user skill exchanges</p>
        </div>
                    ...
                    @forelse($trades as $trade)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusColors[$trade->status] }}">
                                    {{ ucfirst($trade->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $trade->requests->count() }} requests</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $trade->created_at->format('M d, Y') }}</div>
                                <div class="text-sm text-gray-500">{{ $trade->created_at->format('h:i A') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('admin.exchanges.show', $trade) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="text-sm text-gray-500">No exchanges found.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
